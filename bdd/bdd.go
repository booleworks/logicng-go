...
// Build creates a newBdd BDD for a given formula.  The variable ordering in this
// case is the order in which the variables occur in the formula.
func Build(fac f.Factory, formula f.Formula) *BDD {
	bdd, _ := BuildWithHandler(fac, formula, nil)
	return bdd
}

// BuildWithHandler creates a newBdd BDD for a given formula with the given
...
func BuildWithHandler(fac f.Factory, formula f.Formula, bddHandler Handler) (bdd *BDD, ok bool) {
	handler.Start(bddHandler)
	varNum := int32(f.Variables(fac, formula).Size())
	kernel := NewKernel(fac, varNum, varNum*30, varNum*20)
	bddIndex, ok := build(fac, formula, kernel, bddHandler)
	if !ok {
		return nil, ok
	} else {
...
// BuildWithVarOrder creates a newBdd BDD for a given formula and a variable
// ordering.
func BuildWithVarOrder(fac f.Factory, formula f.Formula, order []f.Variable) *BDD {
	bdd, _ := BuildWithVarOrderAndHandler(fac, formula, order, nil)
	return bdd
}

// BuildWithVarOrderAndHandler creates a newBdd BDD for a given formula, variable
// ordering, and bddHandler.  The handler can abort the BDD creation based on
// the number of nodes created during the BDD compilation process.  If the BDD compilation
// was aborted, the ok flag is false.
func BuildWithVarOrderAndHandler(
	fac f.Factory,
	formula f.Formula,
	order []f.Variable,
	bddHandler Handler,
...
// BuildWithKernel creates a newBdd BDD for a given formula with a given kernel.
func BuildWithKernel(fac f.Factory, formula f.Formula, kernel *Kernel) *BDD {
	bdd, _ := BuildWithKernelAndHandler(fac, formula, kernel, nil)
	return bdd
}

// BuildWithKernelAndHandler creates a newBdd BDD for a given formula with a given
// kernel and bddHandler.  The handler can abort the BDD creation based on the
// number of nodes created during the BDD compilation process.  If the BDD compilation
// was aborted, the ok flag is false.
func BuildWithKernelAndHandler(
	fac f.Factory,
	formula f.Formula,
	kernel *Kernel,
	bddHandler Handler,
...
// BuildLiterals creates a newBdd BDD for a conjunction of literals with a given
// kernel.
func BuildLiterals(literals []f.Literal, kernel *Kernel) *BDD {
	var bdd int32
	if len(literals) == 0 {
		bdd = bddFalse
...
func build(fac f.Factory, formula f.Formula, kernel *Kernel, handler Handler) (int32, bool) {
	switch formula.Sort() {
	case f.SortFalse:
	...
	case f.SortNot:
		...
		operand, ok := build(fac, op, kernel, handler)
		if !ok {
			return 0, false
		}
	...
	case f.SortImpl, f.SortEquiv:
		...
		left, ok := build(fac, l, kernel, handler)
		if !ok {
			return 0, false
		}
		right, ok := build(fac, r, kernel, handler)
		if !ok {
			return 0, false
		}
		var res int32
	...
	case f.SortAnd, f.SortOr:
		...
		res, ok := build(fac, ops[0], kernel, handler)
		if !ok {
			return 0, false
		}
		for i := 1; i < len(ops); i++ {
			operand, ok := build(fac, ops[i], kernel, handler)
			if !ok {
				return 0, false
			}
			previous := res
	...
	case f.SortCC, f.SortPBC:
		return build(fac, normalform.NNF(fac, formula), kernel, handler)
	default:
...
func (b *BDD) Restrict(restriction ...f.Literal) *BDD {
	resBdd := BuildLiterals(restriction, b.Kernel)
...
func (b *BDD) Exists(variable ...f.Variable) *BDD {
	resBdd := BuildLiterals(f.VariablesAsLiterals(variable), b.Kernel)
...
func (b *BDD) ModelWithVariables(defaultValue bool, variable ...f.Variable) (*model.Model, error) {
	bdd := BuildLiterals(f.VariablesAsLiterals(variable), b.Kernel)
	var pol int32
	if defaultValue {
		pol = bddTrue
	} else {
